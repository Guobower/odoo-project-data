...
class SampleDevelopmentReport(models.AbstractModel):
    ...
    def render_html(self,docids, data=None):
        ...
        def get_sale():
            value = 0 
            sale = self.env['account.invoice'].search([('partner_id.id','=',record_wizard.customer.id),('type','in',('out_invoice','out_refund'))])
            for x in sale:
        ...
        def get_month_values():
            ...
            date = datetime.strptime(date, "%Y-%m-%d %H:%M:%S.%f")
            payments =  self.env['customer.payment.bcube'].search([('partner_id.id','=',record_wizard.customer.id),('receipts','=',True)])
            sales =  self.env['account.invoice'].search([('partner_id.id','=',record_wizard.customer.id),('type','in',('out_invoice','out_refund'))])
            all_values = []
            sale_values = []
            for x in range(0,5):
                months = date + relativedelta(months=-x-1)
